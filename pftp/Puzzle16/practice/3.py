...
def longestTime(courses):
    longest_selected = []
    if len(courses) == 0:
        return []
    for i in courses:
        after_courses = [x for x in courses if x[0] >= i[1]]
        selected = [i] + longestTime(after_courses)

        if len(longest_selected) == 0:
            longest_selected = selected
        else:
            longest_selected = selected if (sum(x[1] - x[0] for x in longest_selected) < sum(y[1]-y[0] for y in selected)) else longest_selected
    return longest_selected

mycourses2 = [[8, 9], [9, 12], [11, 12], [12, 13], [
    15, 16], [16, 17], [18, 20], [17, 19], [13, 20]]
scourses = [[8, 10], [9, 10], [10, 12], [11, 12], [12, 14], [13, 14]]

print('Earliest finish time:',longestTime(scourses))
