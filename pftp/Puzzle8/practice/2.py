...
def groupMember(dislikePairs, gtuples):
    dislike_list = set(sum(dislikePairs,[]))
    safety_member = []
    unsafety_member = []
    for g in gtuples:
        for name in dislike_list:
            if g[0] == name:
                unsafety_member.append(g)
                break
        else:
            
            safety_member.append(g)

    return safety_member, unsafety_member
        

def InviteDinnerOptimized(guestList, dislikePairs):
    safety_member, unsafety_member = groupMember(dislikePairs, guestList)
    n = len(unsafety_member)
    invite = []
    for i in range(2**n):
        Combination = []
        num = i
        for j in range(n): 
            if (num % 2 == 1):
                Combination = [unsafety_member[n-1-j]] + Combination
            num = num // 2
