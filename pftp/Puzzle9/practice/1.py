...
def optimizeCandList(candList, candTalents):
    optimize_candList = []
    optimize_candTalents = []
    for i in range(len(candList)):
        for j in range(len(candList)):
            if i == j:
                continue
            if len(set(candTalents[i]) - set(candTalents[j])) == 0:
                break
        else:
            optimize_candList.append(candList[i])
            optimize_candTalents.append(candTalents[i])
    return optimize_candList, optimize_candTalents
...
def Hire4Show(candList, candTalents, talentList):
    candList, candTalents = optimizeCandList(candList, candTalents)
    n = len(candList)
    hire = candList[:]
    for i in range(2**n):
        Combination = []
