...
def minHire(comb, hire, checkList, candList, candTalents, talentList):
    if len(checkList) == 0:
        ...
        cand = checkList.pop()
        removeComb = comb[:]
        removeComb.remove(cand)
        if (Good(removeComb, candList, candTalents, talentList)):
            hire = minHire(removeComb, hire, checkList, candList, candTalents, talentList)
        return minHire(comb, hire, checkList, candList, candTalents, talentList)
...
def Hire4Show(candList, candTalents, talentList):
    hire = candList[:]
    checkList = candList[:]
    hire = minHire(candList, hire, checkList, candList, candTalents, talentList)
